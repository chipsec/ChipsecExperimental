...
"""
CHIPSEC includes functionality for reading and writing the SPI flash.
When an image file is created from reading the SPI flash, this image can be parsed to reveal sections, files, variables, etc.

.. warning:: Particular care must be taken when using the spi write and spi erase functions.
These could make your system unbootable.
...
class SPICommand(BaseCommand):
    ...
    def spi_dump(self):
        ...
        # @ TODO: currently assuming at bus 0
        busNo = 0
        (base, limit, _) = self._spi.get_SPI_region(BIOS, busNo)
        if base and limit:
            spi_size = limit + 1
            ...
            buf = self._spi.read_spi_to_file(0, spi_size, self.out_file, busNo)
        if (buf is None):
    ...
    def spi_read(self):
        # @ TODO: currently assuming at bus 0
        busNo = 0
        self.logger.log("[CHIPSEC] Reading 0x{:x} bytes from SPI Flash starting at FLA = 0x{:X}".format(
            self.length, self.spi_fla))
        self.logger.log("[CHIPSEC] {}".format(self._msg))
        buf = self._spi.read_spi_to_file(self.spi_fla, self.length, self.out_file, busNo)
        if (buf is None):
    ...
    def spi_write(self):
        ...
        if not os.path.exists(self.filename):
            ...
            return
        self.logger.log("[CHIPSEC] Writing to SPI flash memory at FLA = 0x{:X} from '{:64s}'".format(
            self.spi_fla, self.filename))

        if self._spi.write_spi_from_file(self.spi_fla, self.filename, busNo):
    ...
    def spi_erase(self):
        ...
        if self._spi.erase_spi_block(self.spi_fla, busNo):
    ...
    def spi_jedec(self):
        if self.option.lower() == 'decode':
            jid = self._spi.get_SPI_JEDEC_ID_decoded()
            for (jedec, man, part) in jid:
            ...
            jid = self._spi.get_SPI_JEDEC_ID()
            for jedec_id in jid:
