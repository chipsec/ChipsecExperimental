...
class MMIOCommand(BaseCommand):
    ...
    def requires_driver(self):
        ...
        if self.func in [self.dump_bar_abs, self.read_abs, self.write_abs]:
    ...
    def dump_bar(self):
        ...
        for bus in bus_data:
            if bus is not None:
                ...
                self.logger.log("Checking for a fixed address")
            try:
                (bar_base, bar_size) = self.cs.mmio.get_MMIO_BAR_base_address(self.bar_name.upper(), bus)
            except CSReadError as msg:
                self.logger.log(msg)
                break
            ...
            iospace = self.cs.mmio.dump_MMIO(bar_base, bar_size)
            pretty_print_hex_buffer(iospace)
    ...
    def dump_bar_abs(self):
        ...
        iospace = self.cs.mmio.dump_MMIO(tmp_base, tmp_length)
        pretty_print_hex_buffer(iospace)

    def read_bar(self):
        bar = self.bar_name.upper()
        bar_base, bar_size = self.cs.mmio.get_MMIO_BAR_base_address(bar, self.bus)
        reg = self.cs.mmio.read_MMIO_reg(bar_base, self.offset, self.width, bar_size)
