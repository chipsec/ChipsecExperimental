...
class MsgBusCommand(BaseCommand):
    ...
    def msgbus_read(self):
        ...
        res = self.cs.msgbus.msgbus_reg_read(self.port, self.reg)
        if res is not None:
    ...
    def msgbus_write(self):
        ...
        self.cs.msgbus.msgbus_reg_write(self.port, self.reg, self.val)
        return
    ...
    def msgbus_message(self):
        ...
        res = self.cs.msgbus.msgbus_send_message(self.port, self.reg, self.opcode, self.val)
        if res is not None:
            self.logger.log("[CHIPSEC] Result: 0x{:08X}".format(res))
