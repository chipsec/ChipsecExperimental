...
class RegisterCommand(BaseCommand):
    ...
    def requires_driver(self):
        ...
        parser_find.add_argument('vid', type=str, nargs="?", default="*", help="Vendor ID - use '*' for wildcard")
        parser_find.add_argument('dev_name', type=str, nargs="?", default="*", help="Device Name - use '*' for wildcard")
        parser_find.add_argument('reg_name', type=str, nargs="?", default="*", help="Register Name - use '*' for wildcard")
    ...
    def reg_find(self):
        print(self.vid, self.dev_name, self.reg_name, self.field_name)
        matches = self.cs.Cfg.get_REGISTERS_match("{}.{}.{}.{}".format(self.vid, self.dev_name, self.reg_name, self.field_name))
        self.logger.log(matches)

    def reg_read(self):
        matches = self.cs.Cfg.get_REGISTERS_match(self.reg_name)  # matches contains fields and registers
    ...
    def reg_read_field(self):
        matches = self.cs.Cfg.get_REGISTERS_match(self.field_name)
        if not matches:
        ...
        for field in matches:
            ...
            values = self.cs.read_register_field(reg_name, field_name)
            for regdata in values:
                self.logger.log("[CHIPSEC] {}.{}=0x{:X} instance:{}".format(reg_name, field_name, regdata.value, regdata.instance))

    def reg_write(self):
        matches = self.cs.get_REGISTERS_match(self.reg_name)
        for reg in matches:
            self.logger.log("[CHIPSEC] Writing {} < 0x{:X}".format(reg, self.value))
            self.cs.write_register(reg, [self.value])
    ...
    def reg_get_control(self):
        if self.cs.Cfg.is_control_defined(self.control_name):
            values = self.cs.get_control(self.control_name)
            for val, instance in values:
                self.logger.log("[CHIPSEC] {} = 0x{:X} - instance:{}".format(self.control_name, val, instance))
        else:
