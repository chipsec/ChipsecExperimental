...
class SPDCommand(BaseCommand):
    ...
    def spd_dump(self):
        if self.dev is not None:
            spd_addr = self.check_input()
            if spd_addr is None:
                self.logger.log('Unable to Dump device {}'.format(self.dev))
                return
            else:
                if not self._spd.isSPDPresent(spd_addr):
                    self.logger.log("[CHIPSEC] SPD for DIMM 0x{:X} is not found".format(spd_addr))
                    return
                self._spd.decode(spd_addr)
        else:
            _dimms = self._spd.detect()
            for _dimm in _dimms:
    ...
    def spd_read(self):
        spd_addr = self.check_input()
        if spd_addr is None:
            self.logger.log('Unable to Read device {}'.format(self.dev))
            return
        if not self._spd.isSPDPresent(spd_addr):
            self.logger.log("[CHIPSEC] SPD for DIMM 0x{:X} is not found".format(spd_addr))
            return

        val = self._spd.read_byte(self.off, spd_addr)
    ...
    def spd_write(self):
        ...
        self._spd.write_byte(self.off, self.val, spd_addr)

    def check_input(self):
        _dev = self.dev.upper()
        if _dev in spd.SPD_DIMM_ADDRESSES:
            spd_addr = spd.SPD_DIMM_ADDRESSES[_dev]
        else:
            try:
                spd_addr = int(_dev, 16)
            except ValueError:
                spd_addr = None
        return spd_addr
