...
from chipsec.lib.uefi_common import EFI_GUID_SIZE, EFI_GUID_FMT, FWType, fw_types
...
def build_efi_modules_tree(fwtype, data, Size, offset, polarity):
    ...
    while sec is not None:
        ...
        if sec.Type in EFI_SECTIONS_EXE:
            ...
            if len(sec.Image) < sec.HeaderSize + EFI_GUID_DEFINED_SECTION_size:
                ...
                guid0, sec.DataOffset, sec.Attributes = struct.unpack(
                    EFI_GUID_DEFINED_SECTION, sec.Image[sec.HeaderSize:sec.HeaderSize + EFI_GUID_DEFINED_SECTION_size])
            ...
            if sec.Guid == EFI_CRC32_GUIDED_SECTION_EXTRACTION_PROTOCOL_GUID:
                sec.children = build_efi_modules_tree(
                    fwtype, sec.Image[sec.DataOffset:], Size - sec.DataOffset, 0, polarity)
                ...
                if sec.Guid == LZMA_CUSTOM_DECOMPRESS_GUID:
                    ...
                    d = decompress_section_data(
                        "", sec_fs_name, sec.Image[sec.DataOffset:], COMPRESSION_TYPE_EFI_STANDARD)
                if d is None:
                    sec.add_comments("Unable to decompress image")
                    d = decompress_section_data(
                        "", sec_fs_name, sec.Image[sec.HeaderSize + EFI_GUID_DEFINED_SECTION_size:],
                ...
                if len(sec.Image) > offset:
                    sec.children = build_efi_modules_tree(
                        fwtype, sec.Image[offset:], len(sec.Image[offset:]), 0, polarity)
            else:
            ...
            for mct in COMPRESSION_TYPES_ALGORITHMS:
                d = decompress_section_data(
                    "", sec_fs_name, sec.Image[sec.HeaderSize + EFI_COMPRESSION_SECTION_size:], mct)
                if d:
                    sec.children = build_efi_modules_tree(fwtype, d, len(d), 0, polarity)
                if sec.children:
                    break
        ...
        elif sec.Type not in SECTION_NAMES:
            sec.children = build_efi_model(sec.Image[sec.HeaderSize:], fwtype)
            if not sec.children:
...
def build_efi_file_tree(fv_img, fwtype):
    ...
    while fwbin is not None:
        fwbin.calc_hashes()
        fvTypes = (EFI_FV_FILETYPE_ALL, EFI_FV_FILETYPE_RAW, EFI_FV_FILETYPE_FFS_PAD)
        fvStates = (EFI_FILE_HEADER_CONSTRUCTION, EFI_FILE_HEADER_INVALID, EFI_FILE_HEADER_VALID)
        if fwbin.Type not in fvTypes or fwbin.State not in fvStates:
