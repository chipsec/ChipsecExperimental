...
class EFI_FV(EFI_MODULE):
    ...
    def __str__(self):
        schecksum = ('{:04X}h ({:04X}h) *** checksum mismatch ***'.format(
            self.Checksum, self.CalcSum)) if self.CalcSum != self.Checksum else ('{:04X}h'.format(self.Checksum))
...
def NextFwVolume(buffer, off=0):
    ...
    while ((fof + vf_header_size) < size):
        ...
        if (ValidateFwVolumeHeader(ZeroVector, FsGuid, FvLength, HeaderLength, ExtHeaderOffset, Reserved, size)):
            return EFI_FV(fof, FsGuid, FvLength, Attributes, HeaderLength,
                          Checksum, ExtHeaderOffset, buffer[fof:fof + FvLength], CalcSum)
        else:
            fof += 0x2C
    return None
...
def GetFvHeader(buffer, off=0):
    ...
    _, FileSystemGuid0, FvLength, _, Attributes, HeaderLength, Checksum, ExtHeaderOffset, Reserved, Revision = struct.unpack(
        EFI_FIRMWARE_VOLUME_HEADER, buffer[off:off + struct.calcsize(EFI_FIRMWARE_VOLUME_HEADER)])
    numblocks, lenblock = struct.unpack(
        EFI_FV_BLOCK_MAP_ENTRY, buffer[fof:fof + struct.calcsize(EFI_FV_BLOCK_MAP_ENTRY)])
    logger().log_hal('{}'.format('''
        ...
        \tReserved:         0x{:02X}
        FFS Guid:    {}
        '''.format(fof, FvLength, Attributes, HeaderLength, Checksum, Revision, ExtHeaderOffset,
                   Reserved, UUID(bytes_le=FileSystemGuid0))))
...
def NextFwFile(FvImage, FvLength, fof, polarity):
    ...
    # if (fof + file_header_size) <= min(FvLength, len(FvImage)):
        ...
        CalcSum = (hsum | (fsum << 8))
        res = EFI_FILE(cur_offset, Name, Type, Attributes, State, IntegrityCheck,
                       fsize, FvImage[cur_offset:cur_offset + fsize], header_size, update_or_deleted, CalcSum)
        break
    return res
    ...
    while curr_offset + EFI_COMMON_SECTION_HEADER_size < ssize:
        ...
        if Size == 0xFFFFFF and (curr_offset + EFI_COMMON_SECTION_HEADER_size + struct.calcsize("I")) < ssize:
            Size = struct.unpack(
                "I",
                sections[
                    curr_offset + EFI_COMMON_SECTION_HEADER_size:
                    curr_offset + EFI_COMMON_SECTION_HEADER_size + struct.calcsize("I")])[0]
            Header_Size = EFI_COMMON_SECTION_HEADER_size + struct.calcsize("I")
        if Type in SECTION_NAMES:
