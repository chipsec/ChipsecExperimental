...
def print_pci_devices(_devices):
    ...
    for (b, d, f, vid, did, rid) in _devices:
        ...
        logger().log("{:02X}:{:02X}.{:X} | {:04X}:{:04X}:{:02X} | {:28} | {}".format(
            b, d, f, vid, did, rid, vendor_name, device_name))
...
def print_pci_XROMs(_xroms):
    ...
    logger().log("-------------------------------------------------")
    for xrom in _xroms:
        logger().log("{:02X}:{:02X}.{:X} | {:04X}:{:04X} | {:08X}  | {:08X}  | {:d}".format(
            xrom.bus, xrom.dev, xrom.fun, xrom.vid, xrom.did, xrom.base, xrom.size, xrom.en))
