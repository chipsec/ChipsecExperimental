...
class HEST (ACPI_TABLE):
    ...
    def parseErrEntry(self, table_content):
        mtype = struct.unpack('<H', table_content[0:2])[0]
        if mtype == 0:  # Arch Machine Check Execption Structure
            return self.parseAMCES(table_content)
        elif mtype == 1:  # Arch Corrected Mach Check Structure or ArchitectureDeferred machine Check Structure
            return self.parseAMCS(table_content, mtype)
        elif mtype == 2:  # NMI Error Structure
            return self.parseNMIStructure(table_content)
        elif mtype == 6 or mtype == 7 or mtype == 8:  # PCIe Root Port AER Structure or PCIe Device AER Structure or PCIe Bridge AER Structure
            return self.parsePCIe(table_content, mtype)
        elif mtype == 9 or mtype == 10:  # Generic hardware Error Source Structure or Generic Hardware Error Source version 2
            return self.parseGHESS(table_content, mtype)
        else:
            pass
  ...
  Architecture Machine Check Exception Structure
    ...
    def parseAMCS(self, table_content, mtype):
        sourceID = struct.unpack('<H', table_content[2:4])[0]
        reserved1 = struct.unpack('<H', table_content[4:6])[0]
        flags = struct.unpack('<B', table_content[6:7])[0]
        enabled = struct.unpack('<B', table_content[7:8])[0]
  ...
  Architecture NMI Error Structure
    ...
    def parsePCIe(self, table_content, mtype):
        sourceID = struct.unpack('<H', table_content[2:4])[0]
        reserved1 = struct.unpack('<H', table_content[4:6])[0]
        flags = struct.unpack('<B', table_content[6:7])[0]
        enabled = struct.unpack('<B', table_content[7:8])[0]
  ...
  {}
    ...
    def parseGHESS(self, table_content, mtype):
        sourceID = struct.unpack('<H', table_content[2:4])[0]
