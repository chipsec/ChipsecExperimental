...
from chipsec.helper.oshelper import helper as chiphelper
...
class sinkhole(BaseModule):
    ...
    def is_supported(self):
        if not (chiphelper().is_windows() or chiphelper().is_linux()):
            ...
            return False
        elif not self.cs.Cfg.is_register_defined('IA32_APIC_BASE') or \
            not self.cs.Cfg.is_register_defined('IA32_SMRR_PHYSBASE') or \
                not self.cs.Cfg.is_register_defined('IA32_SMRR_PHYSMASK'):
    ...
    def check_LAPIC_SMRR_overlap(self):
        smrr_physbase_msr = self.cs.read_register('IA32_SMRR_PHYSBASE', 0)[0]
        apic_base_msr = self.cs.read_register('IA32_APIC_BASE', 0)[0]
        self.cs.print_register('IA32_APIC_BASE', apic_base_msr)
        self.cs.print_register('IA32_SMRR_PHYSBASE', smrr_physbase_msr)

        smrrbase = self.cs.get_register_field('IA32_SMRR_PHYSBASE', smrr_physbase_msr.value, 'PhysBase')
        smrr_base = self.cs.get_register_field('IA32_SMRR_PHYSBASE', smrr_physbase_msr.value, 'PhysBase', True)
        apic_base = self.cs.get_register_field('IA32_APIC_BASE', apic_base_msr.value, 'APICBase', True)
        ...
        res = self.cs.write_register_field('IA32_APIC_BASE', 'APICBase', smrrbase, preserve_field_position=False, instance=0)

        if res is None:
            self.logger.log_important("Error encountered when attempting to modify IA32_APIC_BASE")

        apic_base_msr_new = self.cs.read_register('IA32_APIC_BASE', 0)[0]
        self.logger.log("[*] New IA32_APIC_BASE: 0x{:016X}".format(apic_base_msr_new.value))

        if apic_base_msr_new.value == apic_base_msr.value:
            ...
            self.cs.write_register('IA32_APIC_BASE', apic_base_msr.value, 0)
            self.logger.log("[*] Restored original value 0x{:016X}".format(apic_base_msr.value))
