...
class spi_lock(BaseModule):
    ...
    def is_supported(self):
        if self.cs.Cfg.is_control_defined('FlashLockDown'):
            return True
    ...
    def check_spi_lock(self):
        ...
        reg_print = True
        if self.cs.Cfg.is_control_defined('SpiWriteStatusDis'):
            wrsdis = self.cs.get_control('SpiWriteStatusDis', with_print=reg_print)
            if self.cs.is_all_value(wrsdis, 1):
        ...
        if self.cs.is_all_value(flockdn, 1):
