...
class bios_kbrd_buffer(BaseModule):
    def __init__(self):
        super(bios_kbrd_buffer, self).__init__()
    ...
    def check_BIOS_keyboard_buffer(self):
        ...
        self.logger.log_verbose("[*] Keyboard buffer head pointer = 0x{:X} (at 0x41A), tail pointer = 0x{:X} (at 0x41C)".format(
            kbrd_buf_head, kbrd_buf_tail))
        bios_kbrd_buf = self.cs.mem.read_physical_mem(0x41E, 32)
        self.logger.log_verbose("[*] Keyboard buffer contents (at 0x41E):")
