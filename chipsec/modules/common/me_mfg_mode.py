...
class me_mfg_mode(BaseModule):
    ...
    def __init__(self):
        super(me_mfg_mode, self).__init__()
        self.cs.set_scope({
            "MEI1": "8086.MEI1",
            "HFS": "8086.MEI1.HFS",
        })
    ...
    def is_supported(self):
        if self.cs.Cfg.get_device_bus("MEI1") != []:
            return True
        else:
            self.res = ModuleResult.NOTAPPLICABLE
            self.logger.log_information("ME is Not Supported")
            return False
    ...
    def check_me_mfg_mode(self):
        ...
        if self.logger.VERBOSE:
            for regData in me_hfs_reg:
                self.cs.print_register('HFS', regData)
        me_mfg_mode_mask = self.cs.get_register_field_mask('HFS', 'MFG_MODE', True)

        if self.cs.is_all_value(me_hfs_reg, 0, me_mfg_mode_mask):
