...
class cpu_info(BaseModule):
    ...
    def is_supported(self):
        if self.cs.Cfg.register_has_field('IA32_BIOS_SIGN_ID', 'Microcode'):
            return True
    ...
    def run(self, module_argv):
        ...
        signList = self.cs.read_register_field('IA32_BIOS_SIGN_ID', 'Microcode')

        for regdata in signList:
            ...
            if not os_helper().is_efi():
                self.cs.helper.set_affinity(regdata.instance)

            # Display thread
            self.logger.log('[*] Thread {:04d}'.format(regdata.instance))
            ...
            self.logger.log('[*]            Microcode: {:08X}'.format(regdata.value))
            self.logger.log('[*]')
