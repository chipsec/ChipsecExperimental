...
class smrr(BaseModule):
    ...
    def is_supported(self):
        mtrr_exist = self.cs.Cfg.is_register_defined('MTRRCAP')
        pbase_exist = self.cs.Cfg.is_register_defined('IA32_SMRR_PHYSBASE')
        pmask_exist = self.cs.Cfg.is_register_defined('IA32_SMRR_PHYSMASK')
    ...
    def check_SMRR(self, do_modify):
        ...
        for regdata in msr_smrrbase:
            if self.logger.VERBOSE:
                self.cs.print_register("IA32_SMRR_PHYSBASE", regdata)
            smrrbase = self.cs.get_register_field('IA32_SMRR_PHYSBASE', regdata.value, 'PhysBase', True)
            smrrtype = self.cs.get_register_field('IA32_SMRR_PHYSBASE', regdata.value, 'Type')
        ...
        for regdata in msr_smrrmask:
            if self.logger.VERBOSE:
                self.cs.print_register('IA32_SMRR_PHYSMASK', regdata)
            smrrmask = self.cs.get_register_field('IA32_SMRR_PHYSMASK', regdata.value, 'PhysMask', True)
            smrrvalid = self.cs.get_register_field('IA32_SMRR_PHYSMASK', regdata.value, 'Valid')
        ...
        if self.cs.is_all_value(msr_smrrmask, msr_smrrmask[0].value) and self.cs.is_all_value(msr_smrrbase, msr_smrrbase[0].value):
        ...
        if ok:
            self.logger.log_good("SMRR reads are blocked in non-SMM mode")  # return all F's
        else:
            self.logger.log_bad("SMRR reads are not blocked in non-SMM mode")  # all F's are not returned
