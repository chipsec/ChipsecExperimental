...
class rtclock(BaseModule):
    ...
    def is_supported(self):
        if self.cs.is_core():
            if self.cs.Cfg.is_register_defined('RC'):
                return True
    ...
    def check_rtclock(self):
        ll = ul = 0
        rc_reg = self.cs.read_register('RC')[0]
        check_config_regs = rc_reg.value != 0xFFFFFFFF

        if check_config_regs:
            self.cs.print_register('RC', [rc_reg])
            ll = self.cs.get_register_field('RC', rc_reg.value, 'LL')
            ul = self.cs.get_register_field('RC', rc_reg.value, 'UL')
