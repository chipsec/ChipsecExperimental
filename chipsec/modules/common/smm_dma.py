...
class smm_dma(BaseModule):
    ...
    def is_supported(self):
        ...
        if self.cs.is_atom():
            ...
            return False
        elif not self.cs.Cfg.is_control_defined('TSEGBaseLock') or not self.cs.Cfg.is_control_defined('TSEGLimitLock'):
    ...
    def check_tseg_locks(self):
        ...
        base_lock = self.cs.is_all_value(tseg_base_lock, 1)
        tseg_limit_lock = self.cs.get_control('TSEGLimitLock')
        limit_lock = self.cs.is_all_value(tseg_limit_lock, 1)
        ia_untrusted = False
        if self.cs.Cfg.register_has_field('MSR_BIOS_DONE', 'IA_UNTRUSTED'):
            bios_done = self.cs.read_register_field('MSR_BIOS_DONE', 'IA_UNTRUSTED')
            ia_untrusted = self.cs.is_all_value(bios_done, 1)
    ...
    def check_tseg_config(self):
        ...
        tseg_entries = self.cs.cpu.get_TSEG()
        for (tseg_base, tseg_limit, tseg_size) in tseg_entries:
