...
class bios_smi(BaseModule):
    ...
    def __init__(self):
        super(bios_smi, self).__init__()

    def is_supported(self):
        if not self.cs.Cfg.is_control_defined('SmmBiosWriteProtection') or \
           not self.cs.Cfg.is_control_defined('TCOSMILock') or \
           not self.cs.Cfg.is_control_defined('SMILock') or \
           not self.cs.Cfg.is_control_defined('BiosWriteEnable'):
    ...
    def check_SMI_locks(self):
        ...
        #
        smm_bwp = self.cs.get_control('SmmBiosWriteProtection', with_print=self.logger.VERBOSE)
        if self.cs.is_any_value(smm_bwp, 0):
        ...
        #
        if self.cs.Cfg.is_control_defined('TCOSMIEnable') and self.cs.Cfg.is_control_defined('GlobalSMIEnable'):
            self.logger.log("[*] Checking SMI enables..")
            tco_en = self.cs.get_control('TCOSMIEnable', with_print=self.logger.VERBOSE)
            gbl_smi_en = self.cs.get_control('GlobalSMIEnable', with_print=self.logger.VERBOSE)

            if not self.cs.is_all_value(gbl_smi_en, 1):
                ok = False
                self.logger.log_bad("Global SMI is not enabled")
            elif not (self.cs.is_all_value(tco_en, 1) and self.cs.is_all_value(smm_bwp, 1)):
                warn = True
        ...
        # Checking TCO_LOCK
        #
        if self.cs.is_control_valid('TCOSMILock'):
            tco_lock = self.cs.get_control('TCOSMILock', with_print=self.logger.VERBOSE)
            if not self.cs.is_all_value(tco_lock, 1):
                ok = False
        ...
        # Checking SMI_LOCK
        #
        smi_lock = self.cs.get_control('SMILock', with_print=self.logger.VERBOSE)
        if not self.cs.is_all_value(smi_lock, 1):
            ok = False
