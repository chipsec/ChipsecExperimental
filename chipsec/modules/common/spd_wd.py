...
class spd_wd(BaseModule):
    ...
    def __init__(self):
        ...
        self.cs.set_scope({
            "SMBUS_HCFG": "8086.SMBUS.SMBUS_HCFG",
            "SMBUS": "8086.SMBUS"
        })

    def is_supported(self):
        _bus = self.cs.Cfg.get_device_bus("SMBUS")
        if _bus:
            if self.cs.Cfg.register_has_field('SMBUS_HCFG', 'SPD_WD'):
                return True
            else:
    ...
    def check_spd_wd(self):
        ...
        spd_wd_mask = self.cs.get_register_field_mask('SMBUS_HCFG', "SPD_WD", True)
        if self.logger.VERBOSE:
            for regdata in spd_wd_reg:
                self.cs.print_register("SMBUS_HCFG", regdata)

        if self.cs.is_all_value(spd_wd_reg, spd_wd_mask & 0xFFFFFFFF, spd_wd_mask):
