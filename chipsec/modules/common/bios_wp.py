...
class bios_wp(BaseModule):
    ...
    def is_supported(self):
        ble_exists = self.cs.Cfg.is_control_defined('BiosLockEnable')
        bioswe_exists = self.cs.Cfg.is_control_defined('BiosWriteEnable')
        smmbwp_exists = self.cs.Cfg.is_control_defined('SmmBiosWriteProtection')
    ...
    def check_BIOS_write_protection(self):
        ...
        bioswe = self.cs.get_control('BiosWriteEnable', with_print=True)
        smmbwp = self.cs.get_control('SmmBiosWriteProtection', with_print=True)

        # Is the BIOS flash region write protected?
        write_protected = False
        if self.cs.is_all_value(ble, 1) and self.cs.is_all_value(bioswe, 0):
    ...
    def check_SPI_protected_ranges(self):
        ...
        for bus in _bus:
            (bios_base, bios_limit, _) = self.spi.get_SPI_region(BIOS, bus)
            self.logger.log('\n[*] Checking Bus {:02X}'.format(bus))
            self.logger.log("[*] BIOS Region: Base = 0x{:08X}, Limit = 0x{:08X}".format(bios_base, bios_limit))

            areas_to_protect = [(bios_base, bios_limit)]

            for j in range(5):
                (base, limit, wpe, _, _, _) = self.spi.get_SPI_Protected_Range(j, bus)
                if base > limit:
                    continue
                if wpe == 1:
