...
Reference:
    ...
    <http://www.invisiblethingslab.com/resources/bh08/part2-full.pdf>`_ by Joanna Rutkowska & Rafal Wojtczuk
...
class remap(BaseModule):
    ...
    def is_supported(self):
        if self.cs.is_core():
            if not self.cs.Cfg.is_register_defined('REMAPBASE') or \
               not self.cs.Cfg.is_register_defined('REMAPLIMIT') or \
               not self.cs.Cfg.is_register_defined('TOUUD') or \
               not self.cs.Cfg.is_register_defined('TOLUD') or \
               not self.cs.Cfg.is_register_defined('TSEGMB'):
                self.logger.log_important("Couldn't find definition of required registers (REMAP*, TOLUD, TOUUD, TSEGMB)")
                return False
            return True
    ...
    def check_remap_config(self):
        ...
        remapbase = self.cs.read_register('REMAPBASE')[0].value
        remaplimit = self.cs.read_register('REMAPLIMIT')[0].value
        touud = self.cs.read_register('TOUUD')[0].value
        tolud = self.cs.read_register('TOLUD')[0].value
        tsegmb = self.cs.read_register('TSEGMB')[0].value
        ...
        ia_untrusted = 0
        if self.cs.Cfg.register_has_field('MSR_BIOS_DONE', 'IA_UNTRUSTED'):
            ia_untrusted = self.cs.read_register_field('MSR_BIOS_DONE', 'IA_UNTRUSTED')[0].value
