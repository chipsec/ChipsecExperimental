...
class sgx_check(BaseModule):
    ...
    def check_sgx_config(self):
        ...
        sgx_en_field = self.cs.read_register_field('IA32_FEATURE_CONTROL', 'SGX_GLOBAL_EN', False)
        bios_feature_control_enable = self.cs.is_all_value(sgx_en_field, 1)
        ...
        prmrr_enable = False
        #for tid in range(self.cs.msr.get_cpu_thread_count()):
        mtrrcap = self.cs.read_register_field('MTRRCAP', 'PRMRR', False)
        for mtrr in mtrrcap:
            if mtrr.value == 0:
                self.logger.log_verbose("[*] CPU{:d} Protected Memory Range configuration is not supported".format(mtrr.instance))
            else:
                prmrr_enable = True
                self.logger.log_verbose("[*] CPU{:d} Protected Memory Range configuration is supported".format(mtrr.instance))
        if prmrr_enable:
        ...
        prmrr_uniform = True
        prmrr_locked = True
        check_uncore_vals = self.cs.Cfg.is_register_defined('PRMRR_UNCORE_PHYBASE') and self.cs.Cfg.is_register_defined('PRMRR_UNCORE_MASK')
        for tid in range(self.cs.msr.get_cpu_thread_count()):
