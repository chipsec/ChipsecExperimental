...
class spi_fdopss(BaseModule):
    ...
    def __init__(self):
        super(spi_fdopss, self).__init__()
        self.cs.set_scope({
            "HSFS": "8086.SPI.HSFS",
        })

    def is_supported(self):
        if not self.cs.Cfg.register_has_field('HSFS', 'FDOPSS'):
    ...
    def check_fd_security_override_strap(self):
        hsfsData = self.cs.read_register('HSFS')
        if self.logger.VERBOSE:
            for hsfs_reg in hsfsData:
                self.cs.print_register('HSFS', hsfs_reg)
        fdopssmask = self.cs.get_register_field_mask('HSFS', 'FDOPSS', True)

        if not self.cs.is_any_value(hsfsData, 0, fdopssmask):
