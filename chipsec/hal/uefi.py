...
from chipsec.lib.uefi_variables import EFI_VAR_DICT
...
class UEFI(HALBase):
    ...
    def set_EFI_variable_from_file(self, name, guid, filename, datasize=None, attrs=None):
        if filename is None:
            self.logger.error('File with EFI variable is not specified')
            return False
    ...
    def get_next_variable(self, name, guid):
        return self.helper.get_next_EFI_variable(name, guid)
