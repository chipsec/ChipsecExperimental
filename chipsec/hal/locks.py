...
class locks(HALBase):
    ...
    def get_locks(self):
        ...
        """
        return self.cs.Cfg.get_lock_list()

    def lock_valid(self, lock_name, instance=None):
        res = 0
        if self.cs.Cfg.is_lock_defined(lock_name):
            res |= LockResult.DEFINED
        try:
            self.cs.Cfg.get_locked_value(lock_name)
            self.cs.get_lock(lock_name, instance)
    ...
    def is_locked(self, lock_name, instance=None):
        """
        Return whether the lock has the value setting
        """
        res = self.lock_valid(lock_name, instance)
        if is_set(res, LockResult.HAS_CONFIG) and is_set(res, LockResult.CAN_READ):
            locked = self.cs.Cfg.get_locked_value(lock_name)
            lock_setting = self.cs.get_lock(lock_name, instance)
            if not self.cs.is_all_value(lock_setting, lock_setting[0].value):
                res |= LockResult.INCONSISTENT
            if self.cs.is_all_value(lock_setting, locked):
