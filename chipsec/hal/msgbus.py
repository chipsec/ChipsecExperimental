...
class MsgBus(hal_base.HALBase):
    ...
    def __MB_MESSAGE_MCR(self, port, reg, opcode):
        mcr = 0x0
        mcr = self.cs.set_register_field('8086.P2SBC.MSG_CTRL_REG', mcr, 'MESSAGE_WR_BYTE_ENABLES', 0xF)
        mcr = self.cs.set_register_field('8086.P2SBC.MSG_CTRL_REG', mcr, 'MESSAGE_ADDRESS_OFFSET', reg)
        mcr = self.cs.set_register_field('8086.P2SBC.MSG_CTRL_REG', mcr, 'MESSAGE_PORT', port)
        mcr = self.cs.set_register_field('8086.P2SBC.MSG_CTRL_REG', mcr, 'MESSAGE_OPCODE', opcode)
        return mcr

    def __MB_MESSAGE_MCRX(self, reg):
        mcrx = 0x0
        mcrx = self.cs.set_register_field('8086.P2SBC.MSG_CTRL_REG_EXT', mcrx, 'MESSAGE_ADDRESS_OFFSET_EXT',
                                          (reg >> 8), preserve_field_position=True)
        return mcrx

    def __MB_MESSAGE_MDR(self, data):
        mdr = 0x0
        mdr = self.cs.set_register_field('8086.P2SBC.MSG_DATA_REG', mdr, 'MESSAGE_DATA', data)
        return mdr
    ...
    def msgbus_write_message(self, port, register, opcode, data):
        ...
        self.logger.log_hal("[msgbus] write: port 0x{:02X} + 0x{:08X} (op = 0x{:02X}) < data = 0x{:08X}".format(
