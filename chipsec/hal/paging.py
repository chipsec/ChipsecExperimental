...
from chipsec.defines import BOUNDARY_4KB, BOUNDARY_2MB, BOUNDARY_1GB, MASK_64b
from chipsec.defines import BIT0, BIT1, BIT2, BIT7
...
class c_translation(object):
    ...
    def is_translation(self, addr, mask, size):
    ...
    def get_translation(self, addr):
        ...
        if self.is_translation(addr, ADDR_4KB, '4KB'):
            result = self.translation[addr & ADDR_4KB]['addr'] | (addr & ~ADDR_4KB)
        elif self.is_translation(addr, ADDR_2MB, '2MB'):
            result = self.translation[addr & ADDR_2MB]['addr'] | (addr & ~ADDR_2MB)
        elif self.is_translation(addr, ADDR_1GB, '1GB'):
