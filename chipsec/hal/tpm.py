...
class TPM(hal_base.HALBase):
    def __init__(self, cs):
        ...
        self.TPM_BASE = (self.cs.Cfg.get_mem_def("8086.TPM")["address"])
        if self.TPM_BASE is None:
            raise TpmRuntimeError
    ...
    def dump_intcap(self, locality):
        ...
        """
        register = '8086.TPM.TPM_INTF_CAPABILITY'
        self.dump_register(register, locality)
    ...
    def dump_intenable(self, locality):
        ...
        """
        register = '8086.TPM.TPM_INT_ENABLE'
        self.dump_register(register, locality)
    ...
    def dump_register(self, register_name, locality):
        register = self.cs.Cfg.get_register_def(register_name)
        base = register['address']
        offset = register['offset'] ^ LOCALITY[locality]
        size = register['size']
        data = self.cs.mmio.read_MMIO_reg(base, offset, size)
        self.log_register_header(register_name, locality)
        self.cs.print_register(register_name, RegData(data, None))
