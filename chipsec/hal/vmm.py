...
class VirtIO_Device(HALBase):
    ...
    def set_device(self, b, d, f):
        self.bus = b
        self.dev = d
        self.fun = f

    def dump_device(self):
        if self.bus is not None and self.dev is not None and self.fun is not None:
            ...
            for (bar, isMMIO, _, _, _, size) in bars:
                self.logger.log('')
                if isMMIO:
                    mmio_space = self.cs.mmio.dump_MMIO(bar, size)
                    pretty_print_hex_buffer(mmio_space)
                else:
                    io_space = self.cs.io.dump_IO(bar, size, 4)
                    pretty_print_hex_buffer(io_space)
        else:
            self.logger.log_hal("[virtio] no device specified")
