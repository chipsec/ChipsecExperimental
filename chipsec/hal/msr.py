...
##########################################################################################################
    ...
    def read_msr(self, cpu_thread_id, msr_addr):
        ...
        self.logger.log_hal("[cpu{:d}] RDMSR( 0x{:x} ): EAX = 0x{:08X}, EDX = 0x{:08X}".format(
    ...
    def write_msr(self, cpu_thread_id, msr_addr, eax, edx):
        ...
        self.logger.log_hal("[cpu{:d}] WRMSR( 0x{:x} ): EAX = 0x{:08X}, EDX = 0x{:08X}".format(
            cpu_thread_id, msr_addr, eax, edx))
        return
    ...
    def get_cpu_thread_count(self):
        ...
        if thread_count is None or thread_count < 0:
            self.logger.log_hal("helper.get_threads_count didn't return anything. Reading MSR 0x35 to find out number"
                                "of logical CPUs (use CPUID Leaf B instead?)")
            thread_count = self.cs.read_register_field("IA32_MSR_CORE_THREAD_COUNT", "Thread_Count")[0].value

        if 0 == thread_count:
            self.logger.log_hal("Unable to get thread count defaulting to 1")
            thread_count = 1
