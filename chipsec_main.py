...
from chipsec.lib.display import print_banner, chipsec_properties
...
class ChipsecMain:
    ...
    def run_module(self, modx, module_argv):
        ...
        try:
            ...
            if not self._list_tags:
                logger().log_debug("[*] Module path: {}".format(modx.get_location()))
    ...
    def parse_args(self):
        ...
        parser = argparse.ArgumentParser(usage='%(prog)s [options]',
                                         ...
                                         epilog=chipsec.defines.ExitCode.help_epilog, add_help=False)
        ...
        adv_options.add_argument('--helper', dest='_helper', help='specify OS Helper',
    ...
    def properties(self):
        ...
        ret["Platform"] = "{}, VID: {:4}, DID: {:4}, RID: {:02X}".format(self._cs.Cfg.longname, self._cs.Cfg.vid,
                                                                         self._cs.Cfg.did, self._cs.Cfg.rid)
        if not self._cs.is_atom():
            ret["PCH"] = "{}, VID: {:4}, DID: {:4} RID: {:02X}".format(self._cs.Cfg.pch_longname, self._cs.Cfg.pch_vid,
    ...
    def main(self):
        if self.help:
            return chipsec.defines.ExitCode.OK
        ...
        if self._load_config:
            try:
                ...
                if self.failfast:
                    raise msg
                return chipsec.defines.ExitCode.EXCEPTION
